(function () {
    ...
    angular
        ...
        .component('aonEbConfigForm', {
            templateUrl: '/static/app/common/panels/config-form.component.html',
            transclude: true,
            bindings: {
                entityType: '<',
                securityType: '<',
            ...
            require: {
                apiProvider: '?^aonEbApiProvider'
            },
            controller: controller
        });

    controller.$inject = ['$scope', '$state', '$stateParams', '$attrs', '$parse', '$q', 'aonEbApp', 'flexApi', '$uibModal', 'aonEbCommonModals', 'translationService', 'entitySecurityService', 'giConstant', 'dateUtils', 'flexFilterService']

    function controller($scope, $state, $stateParams, $attrs, $parse, $q, aonEbApp, flexApi, $uibModal, aonEbCommonModals, translationService, entitySecurityService, giConstant, dateUtils, flexFilterService) {
        ...
        $ctrl.$onInit = function () {
            ...
            if (!$ctrl.options.id) {
                $ctrl.options.id = $ctrl.entityType;
            }

            if (!$ctrl.options.title) {
                $ctrl.options.title = $ctrl.entityType;
            }

            if (!$ctrl.options.translationEntity) {
                $ctrl.options.translationEntity = $ctrl.entityType;
            }

            if ($ctrl.options.apiParams) {
                $ctrl.apiParams = $ctrl.options.apiParams;
            }

            if (!angular.isFunction($ctrl.options.read) && $ctrl.apiProvider.api && $ctrl.apiProvider.api[$ctrl.entityType]) {
                if (!$ctrl.routeParams) {
                    buildState();
                }
                $ctrl.defaults = $ctrl.options.defaults ? angular.merge($ctrl.options.defaults, buildDefaults()) : buildDefaults();
                $ctrl.options.read = function () {
                        if ($ctrl.apiParams && $ctrl.apiParams.id === giConstant.NEW_RECORD) {
                            return $ctrl.apiProvider.api[$ctrl.entityType].newEntity($ctrl.defaults);
                        }
                        else {
                            return $ctrl.apiProvider.api[$ctrl.entityType].get($ctrl.apiParams);
                        }
                    }
            }

            if (!angular.isFunction($ctrl.options.save) && $ctrl.apiProvider.api && $ctrl.apiProvider.api[$ctrl.entityType]) {
                if (!$ctrl.routeParams) {
                    buildState();
                }
                $ctrl.options.save = $ctrl.apiParams ? function (dataRecord) {
                    return $ctrl.apiProvider.api[$ctrl.entityType].save({}, dataRecord).then(function (data) {
                        if ($ctrl.apiParams.id === giConstant.NEW_RECORD) {
                            $ctrl.apiParams.id = data.RecordID;
                            var routeParams = {};
                            routeParams[$ctrl.routeParams.Name] = data.RecordID;
                            aonEbApp.routeTo('.', routeParams, { notify: false, reload: false, location: 'replace', inherit: true });
                        }
                        $scope.$emit('EntityChange', $ctrl.entityType, data.RecordID, giConstant.EntityChange.SAVE);
                        return data;
                    });
                } : null;
            }
            ...
            $q.all([
                ...
                flexFilterService.getHiddenFieldsForEntity($ctrl.entityType),
                ...
                if (!$ctrl.options.hiddenFields) {
                    $ctrl.options.hiddenFields = results[i++];
                }   
                $ctrl.labels = {};
        ...
        function refresh(recordId) {
            ...
            return $q.when($ctrl.options.read()).then(function (data) {
                // Copy data to ensure source is preserved
                $ctrl.dataRecord = angular.copy(data);
                $scope.$emit('EntityChange', $ctrl.entityType, $ctrl.dataRecord, giConstant.EntityChange.REFRESH);
                // Check if dataRecord belongs to the current tenant, the user can only view common records.
                var isTenantRecord = !$ctrl.dataRecord.Tenant_RecordID || $ctrl.dataRecord.Tenant_RecordID === aonEbApp.context.Tenant.RecordID;

                // Update action privileges
                $ctrl.canEdit = isTenantRecord && $ctrl.entitySecurity.Privilege_Update && angular.isFunction($ctrl.options.save);
                $ctrl.canDelete = isTenantRecord && $ctrl.entitySecurity.Privilege_Delete && angular.isFunction($ctrl.options.delete);
                ...
                if (!$ctrl.dataRecord.RecordID || $ctrl.dataRecord.RecordID === "NEW") {
                    $ctrl.options.mode = giConstant.EditPanelMode.OPEN_IN_EDIT;
                    $ctrl.canViewAudit = false;
                }
                else {
                    $ctrl.options.mode = giConstant.EditPanelMode.EDITABLE;
                }
        ...
        function buildState() {
            var gridStateName = $state.current.name;
            var posLastSeparator = gridStateName.lastIndexOf('.');
            if (posLastSeparator > 0) {
                var statePrefix = gridStateName.substring(0, posLastSeparator + 1);
                var detailStateName = statePrefix + aonEbApp.camelToKebabCase($ctrl.entityType) + '-details';

                var state = $state.get(detailStateName);
                if (state !== null) {
                    // Matching state found so assume this will open as a new view in the current window.
                    buildRouteParams(state);
                }
            }
        }

        function buildRouteParams(state) {
            // If no route parameters provided build a custom set based on URL
            if (angular.isUndefined($ctrl.routeParams)) {
                ...
                // Search for all path element matching :someThingId and create dataItem binding to SomeThing_RecordID
                $ctrl.routeParams = {};
                _.each(urlParams, function (param) {
                    if (param.length > 2 && param[0] === ':' && param.substring(param.length - 2).toLowerCase() === 'id') {
                        var paramName = param.substring(1);
                        if ($stateParams[paramName]) {
                            $ctrl.routeParams.Name = paramName;
                            if (!$ctrl.apiParams) {
                                $ctrl.apiParams = { id: $stateParams[paramName], parentId: $ctrl.options.apiParams.parentId };
                            }
                        }
                    }
                })
        ...
        function buildDefaults() {
            return {
                Parent_RecordID: $ctrl.apiParams.parentId,
                RecordID: giConstant.NEW_RECORD,
                RecordEffective: aonEbApp.getSessionDate(),
                JITRule_RecordID: '11111111-1111-1111-1111-111111111111'
            }
        }
    };
