// This is a modified Element Modifier from the 'jcs-autoValidate' package.
//  * Handles kendo items 
...
(function (angular) {
    ...
    angular.module('app')
        .factory('giBootstrap3ElementModifier', [
            '$rootScope',
            '$log',
            '$compile',
            function ($rootScope, $log, $compile) {
                var reset = function (el) {
                    ...
                    findFormGroupElement = function (el) {
                        var elems = findWithClassElementAsc(el, 'aon-eb-form-group');
                        if (elems.length === 0) {
                            elems = findWithClassElementAsc(el, 'form-group');
                        }
                        return elems;
                    },
                    ...
                    addValidationStateInlineMessages = true,

                    enableValidationStateInlineMessages = function(enable) {
                        addValidationStateInlineMessages = enable;
                    },
                    ...
                    makeInvalid = function (el, errorMsg) {
                        ...
                        if (frmGroupEl) {
                            ...
                            if (addValidationStateInlineMessages) {
                                insertAfter(inputGroupEl.length > 0 ? inputGroupEl : el, helpTextEl);   
                            }
                            if (addValidationStateIcons) {
                                var iconElText = '<span class="fa fa-exclamation form-control-feedback" uib-popover="' + errorMsg + '" popover-placement="right" popover-trigger="mouseenter"></span>';
                                if (inputGroupEl.length > 0) {
                                    iconElText = iconElText.replace('form-', '');
                                    iconElText = '<span class="input-group-addon control-feedback" uib-popover="' +errorMsg + '" popover-placement="right" popover-trigger="mouseenter">' +iconElText + '</span>';
                                }

                                var iconEl = angular.element(iconElText);

                                $compile(iconEl)($rootScope);

                                insertAfter(el, iconEl);
                            }
                        } else {
                            $log.error('Angular-auto-validate: invalid bs3 form structure elements must be wrapped by a form-group class');
                        }
                ...
                return {
                    ...
                    enableValidationStateInlineMessages: enableValidationStateInlineMessages,
                    key: 'gi-bs3'
                };
            }
        ]);
